...
### Verificado
- **Prioridad D (3 verificaciones técnicas, bajo esfuerzo)**:
  - **Doble-scroll anidado en "Destacados" (Safari/mobile)**: confirmado
    que el carrusel horizontal de 3.2 (`FeaturedCarousel`) sí resuelve el
    bug original de §16 (dos scrolls verticales anidados compitiendo por
    el mismo eje) — el panel pineado y el carrusel scrollean en ejes
    distintos, no duplicados. Quedaba un caso límite sin cubrir: al llegar
    al final del recorrido horizontal, el swipe puede encadenar
    ("scroll chaining") hacia el panel vertical padre en iOS Safari, o
    confundirse con el gesto nativo de "volver atrás" cerca del borde
    izquierdo de la pantalla. Se agregó `overscroll-x-contain` (corta el
    chaining en los bordes del carrusel) y `touch-pan-x` (declara el eje
    del gesto en vez de dejarlo inferir) en `FeaturedCarousel.tsx` — sin
    tocar la estructura del componente ni el drag con mouse existente.
  - **`/dashboard`, ¿necesita teaser en home?**: auditado — no. No es un
    panel de usuario logueado (el sitio no tiene sistema de cuentas): es
    el dashboard interno de monetización, protegido por Basic Auth vía
    `middleware.ts` (`DASHBOARD_PASSWORD`, fail-closed si falta la env
    var) y con `robots: { index: false, follow: false }`. No hay ningún
    link hacia `/dashboard` desde la home ni la navegación pública hoy, y
    así debe quedar — exponer un teaser lo convertiría en una superficie
    pública que anuncia la existencia de un panel administrativo. Sin
    cambios de código; queda documentado acá para que no se reabra como
    pendiente.
  - **Velocidad del marquee de fabricantes, mobile vs. desktop**:
    auditado — ya está resuelto por diseño, no por accidente. Desktop usa
    `animation-duration: 40s`; por debajo de `640px` sube a `70s` (mismo
    recorrido en CSS puro, `globals.css`), es decir que mobile ya
    desplaza a ~57% de la velocidad de desktop en vez de heredar la misma
    duración sobre una franja de pantalla más angosta (que se hubiera
    sentido más rápido, no más lento). Coincide con el criterio "mantener
    auto-scroll, reducir velocidad" documentado en el propio CSS (2.2).
    Sin cambios de código.

  `npm run type-check` y `lint` en verde sobre `FeaturedCarousel.tsx`
  (único archivo modificado de los tres puntos).

### Agregado
- **Prioridad C (4 tickets sueltos de pulido, superficies independientes
  entre sí)**:
  - **Tabs animados en `/rankings`**: el índice pasa de una grilla de
    cards que enlazaban a cada `/rankings/[slug]` a un preview navegable
    con tabs — mismos `RankingResult` ya calculados por
    `getAvailableRankings()` (ningún criterio/orden nuevo), solo se
    resuelven imagen y conteo de relaciones de los vehículos que entran
    en el preview (`RANKING_PREVIEW_COUNT = 10`, no los hasta
    `RANKING_TOP_N` del ranking completo). Nuevo
    `RankingsLeaderboardTabs.tsx`; el cambio de tab remonta el panel
    (`key={active.slug}`) para que la animación de entrada
    (`.rankings-tabs-panel`, `globals.css`) corra siempre. Las páginas
    `/rankings/[slug]` no cambian — siguen siendo el destino de "Ver
    ranking completo" y la fuente de JSON-LD `ItemList`.
  - **Micro-refuerzo visual al guardar un favorito**: `WishlistButton`
    dispara un burst de 6 "chispas" lineales (mismo trazo sin relleno que
    `HeartIcon`, ángulo por chispa vía `--spark-angle`) más un pop breve
    del ícono, solo al *activar* (nunca al sacar de favoritos) — a
    propósito no es confeti genérico de colores random. 100% CSS
    (`.wishlist-burst-spark`/`.wishlist-heart-pop`), con su propio
    apagado bajo `prefers-reduced-motion`; el timer de React que limpia
    el estado (`BURST_DURATION_MS`) no depende de `onAnimationEnd`, así
    que no se cuelga si la animación está desactivada.
  - **Swipe A/B mobile en el comparador en vivo**: `LiveCompareTeaser`
    (home) comprimía los dos títulos + 3 métricas en 3 columnas angostas
    en mobile. Ahora, por debajo de `sm:`, un `MobileCompareSwipe` nuevo
    muestra panel completo por vehículo con swipe táctil horizontal
    (umbral `SWIPE_THRESHOLD_PX`) más un segmentado A/B como alternativa
    sin gesto (accesibilidad/descubribilidad). El layout de 3 columnas
    original queda igual a partir de `sm:` (`hidden sm:block`).
  - **FAQ corta y visual antes del footer de la home**: nuevo
    `HomeFaqPanel` (acordeón, una pregunta abierta a la vez, sin
    librería — `grid-template-rows: 0fr → 1fr` en CSS puro) montado en
    `page.tsx` *después* de `<PinnedScrollStages>`, aprovechando que ahí
    el documento ya volvió a scroll normal (el CTA final del track es el
    último panel fijo) en vez de sumar un décimo `Stage` al crossfade.
    Nuevo `generateFaqJsonLd()` en `lib/seo.ts` (mismo texto que se
    renderiza en pantalla, sin resumen aparte). Una de las 5 preguntas
    cubre el pivote GTA6→AutoFicha (ver README) porque es la que más
    contexto rompe si alguien llega esperando el juego.

  `npm run type-check`, `lint`, `test` (456/456), `verify:content`,
  `verify:min-count`, `verify:seo` y `build` (842 páginas) en verde con
  las 4 superficies activas.

### Experimental
- **Transición FLIP hero → card en Categorías (sin sprint fijo)**: prueba
  de concepto de shared-element transition entre el vehículo mostrado en
  el hero de home y su card correspondiente en `/categorias/[grupo]`,
  vía la View Transitions API nativa del navegador
  (`document.startViewTransition`), sin librería nueva. Nuevo
  `src/lib/view-transitions.ts` (mecanismo + límites documentados
  in-line); `HeroVehicleShowcase` gana un link real hacia la categoría
  del vehículo actual (antes puramente decorativo) que intercepta el
  click solo cuando hay soporte de navegador y no hay
  `prefers-reduced-motion`; `EntityCard` (variante `grid`, solo
  vehículos) consume una marca de `sessionStorage` con TTL corto para
  asignarle el mismo `view-transition-name` a la card de destino. Se
  deja marcado como **experimental** a propósito, no como ítem
  comprometido de FASE 5/Prioridad B: la transición es real entre
  *rutas* de Next.js App Router (no dentro del mismo árbol de
  componentes), Next no expone un callback de "ruta nueva ya pintada"
  para coordinar con la API del navegador (se usa un doble
  `requestAnimationFrame` como heurística) y el soporte de navegador hoy
  es parcial (Chromium/Safari 18+, no Firefox) — sin soporte, el
  comportamiento cae a navegación normal sin ninguna animación ni error.
  `npm run type-check`, `lint`, `test` (456/456) y `build` (842 páginas)
  en verde con el experimento activo.

### Agregado
- **FASE 5 (pulido, prioridad B — backlog continuo)**: 6 tickets sueltos
  de la sección §5 del informe de auditoría, priorizables entre sprints.
  No bloqueaban release; se agrupan acá porque tocan superficies
  distintas del sitio sin depender entre sí:
  - **Dato en vivo en el eyebrow del hero**: `getMostRecentUpdate()`
    (`src/lib/entities.ts`) trae la fecha de actualización más reciente
    de todo el catálogo y se muestra como "Actualizado hace N días"
    junto al nombre del sitio (`formatRelativeTime`, mismo helper que ya
    usa el resto del sitio — cae a fecha absoluta corta después de
    ~30 días). Se degrada a mostrar solo `SITE_NAME` si no hay ninguna
    entidad cargada (caso borde, no pasa en producción).
  - **Placeholder del buscador con ejemplos reales rotando**:
    `pickSearchExamples()` (`vehicle-category.ts`) recorre el catálogo
    completo (no solo `featured`) y arma una muestra de títulos reales,
    uno por categoría distinta, de forma determinista (sin
    `Math.random()`, para no arriesgar mismatch de hidratación
    servidor/cliente) — reemplaza el `DEFAULT_EXAMPLES` hardcodeado que
    tenía `QuickSearchForm` como único fallback.
  - **Filtro rápido inline por carrocería en Categorías**:
    `CategoryQuickFilter` (nuevo componente, `home/`) +
    `computeCategoryQuickFilterOptions()` (`vehicle-category.ts`). Se
    evaluó la alternativa obvia (chip que linkea directo a
    `/categorias/[grupo]`) y se descartó por no ser un filtro real, solo
    navegación más corta — acá el click abre un panel inline con una
    muestra de títulos reales de esa carrocería sin perder el scroll ni
    el stage activo de la home; el link a la página completa de la
    categoría queda como salida explícita.
  - **Excerpt de 1–2 líneas + fecha relativa en cards de Noticias**:
    `EntityCard` gana un prop `dateLabel` (fecha ya formateada en
    servidor, sin `new Date()` en cliente) con ícono de calendario, y la
    descripción pasa a `line-clamp-2` específicamente para
    `EntityType.NEWS` (el resto de los tipos mantiene `line-clamp-3`,
    que ya funcionaba bien para descripciones más largas).
  - **Reposición del `AdUnit` del hero**: el `<AdUnit slotId="3119092668">`
    vivía en la cascada del hero (`Reveal index={3.5}`, panel 1) — el
    primer panel que ve cualquier visitante nuevo y el de mayor peso de
    conversión de todo el sitio (H1, CTAs, buscador). Se evaluó y se
    movió al panel de Noticias, después del grid de últimas noticias:
    panel de solo lectura al que se llega tras recorrer Categorías/
    Comparador/Destacados/Evidencia (protagonismo mucho menor, sin
    competir con ningún CTA), y coherente con el patrón "after-content"
    que ya usan los otros 3 `<AdUnit>` del sitio (ficha, comparador,
    fabricantes — todos después del contenido principal, nunca antes).
    Mismo slot real de AdSense (no se crea uno nuevo); solo cambia
    `dataTrackingLabel` (`ad-hero` → `ad-home-noticias`) para que GA4
    refleje la ubicación real.
  - **Fabricante individual con mini-showcase (confirmado, no nuevo)**:
    se auditó el pedido puntual del ticket ("confirmar/crear
    `/fabricantes/[slug]` con mini-showcase") contra el código real —
    ya existe desde FASE 7, servido por la ruta genérica
    `[entityType]/[slug]/page.tsx` (`EntityType.MANUFACTURER` es un tipo
    más de esa ruta, no necesita una carpeta `fabricantes/[slug]`
    propia). Trae `getManufacturerStats()` (total de modelos, rango de
    potencia, rango de años, categorías presentes) y una grilla de
    modelos destacados a modo de mini-showcase. Se deja documentado acá
    para que quede registrado que se confirmó, no se reconstruyó de
    cero.
  - `npm run type-check`, `npm run lint` (0 errores), `npm test`
    (456/456) y `npm run build` (842 páginas) en verde. Los scripts
    `verify:min-count/content/relations/manufacturer-slugs/reserved-keys/
    seo/adsense-csp` también pasan sin cambios. `verify:tailwind` falla
    por un problema preexistente y no relacionado (`shadow-auto-sm` no
    se usa en ningún componente de `src/`, así que Tailwind lo purga del
    CSS compilado) — no se tocó nada de `boxShadow`/sombras en esta
    fase, queda anotado para una fase posterior en vez de mezclarlo acá.
