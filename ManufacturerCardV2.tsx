...
export function ManufacturerCardV2({ entity, className, relationCount }: ManufacturerCardV2Props) {
  ...
  return (
    // id ancla que debe coincidir con el kebab del nombre generado por
    // ManufacturerArchive (`/fabricantes#<kebab>`): misma transformación
    // `toLowerCase().replace(/\s+/g, '-')` sobre el nombre del fabricante,
    // para que los links del home resuelvan al scroll de la grilla de
    // fabricantes en vez de quedar referencias a un ancla inexistente.
    <div
      className={cn('group/mv2', className)}
