...
export function HeroNewsFlash({ items, intervalMs = 6000 }: HeroNewsFlashProps) {
  ...
  return (
    <div
      className="hero-news-flash glass-surface inline-flex max-w-full items-center gap-2.5 rounded-full border border-auto-border/70 px-4 py-2 hover:-translate-y-0.5 hover:border-auto-accent/60"
    ...
    >
      ...
      >
        <span className="hero-news-flash-badge shrink-0 rounded-full bg-auto-accent px-2.5 py-0.5 text-[0.65rem] font-bold uppercase tracking-wide text-auto-darker">
          Última filtración
        </span>
        ...
        <span key={current.slug} className="hero-news-flash-title truncate text-sm text-auto-text-secondary" aria-live="off">
          {current.title}
        </span>
        <span className="hero-news-flash-arrow shrink-0 text-auto-accent" aria-hidden="true">
          →
        </span>
      </Link>
      {items.length > 1 && (
