...
export function EntityGallery({ images, entityTitle }: EntityGalleryProps) {
      ...
      images.map((img, i) => (
        <button
          ...
          className={cn(
            ...
            i === active ? 'border-auto-accent' : 'border-auto-border hover:border-auto-border-strong'
          )}
          style={{ width: 64 }}
        >
  ...
  const lightbox = open && (
    ...
    >
      ...
      {total > 1 && (
        <p className="absolute bottom-3 left-1/2 z-20 -translate-x-1/2 text-xs text-auto-text-secondary sm:bottom-6">
          {active + 1} / {total}
        </p>
      )}
    </div>
  ...
  return (
    <div className="space-y-2">
      <button
        ...
        className="card-media relative block aspect-[4/5] w-full cursor-zoom-in overflow-hidden rounded-lg border border-auto-border bg-auto-dark"
      >
        <Image
          src={current.src}
          alt={current.alt}
        ...
        <span className="absolute bottom-2 right-2 z-10 rounded-full border border-auto-border bg-black/70 px-2 py-0.5 text-[10px] font-semibold text-auto-text-secondary backdrop-blur-sm">
          {active + 1} / {total}
        </span>
      </button>
