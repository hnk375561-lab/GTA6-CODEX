...
export function WishlistExplorer({ entities, imageBySlug }: WishlistExplorerProps) {
    ...
    return (
      <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 lg:grid-cols-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="aspect-[4/3] animate-pulse rounded-2xl border border-auto-border bg-auto-surface" />
        ))}
      </div>
    )
  }
  ...
  if (count === 0 || saved.length === 0) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-auto-border bg-auto-card px-6 py-16 text-center">
        <p className="text-lg font-semibold text-auto-text">Todavía no guardaste favoritos</p>
        <p className="max-w-sm text-sm text-auto-text-secondary">
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <button
          ...
          className="text-sm font-semibold text-auto-text-secondary transition-colors hover:text-auto-accent-strong"
        >
          Vaciar favoritos
        </button>
      </div>
