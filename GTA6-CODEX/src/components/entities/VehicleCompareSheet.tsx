    ...
    <div className={scrollable ? 'overflow-x-auto' : undefined}>
      <div style={{ minWidth }}>
              ...
              <CompareRow key={row.key} label={row.label}>
                {vehicles.map((v, i) => (
                  <span
                    key={v.slug}
                    className={cn(
                      'inline-flex w-fit items-center gap-1.5 rounded-md text-sm text-auto-text',
        ...
        {(() => {
          ...
          return (
            <div className="mt-8 border-t border-auto-border pt-6">
              <h3 className="mb-4 text-sm font-semibold text-auto-text">Equipamiento</h3>
              <div className="space-y-2">
                {allEquipment.map((equipmentName) => (
                  <div key={equipmentName} className="rounded-lg border border-auto-border bg-auto-surface/40 p-3">
                    <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-auto-text-secondary">{equipmentName}</p>
                    <div
                        ...
                        const bgColor =
                          status === 'present'
                            ...
                            : status === 'absent'
                              ...
                              : 'bg-auto-surface text-auto-text-secondary'
