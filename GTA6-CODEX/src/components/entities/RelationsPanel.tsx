            ...
            {entities.map(({ entity: e, relation }) => {
              ...
              return (
                <li key={`${e.type}-${e.slug}`} className="group/row -mx-2 flex items-center gap-1 rounded-md px-0">
                  <Link
                    href={`/${e.type}/${e.slug}`}
                    className="group flex flex-1 items-center gap-3 rounded-md border border-dashed border-transparent px-2 py-2 transition-colors duration-200 hover:border-auto-border-strong hover:bg-auto-darker/40"
                  >
                    <EntityImage
                      entity={e}
                  ...
                  {showCompare && (
                    <Link
                      href={compareHref}
                      className="shrink-0 rounded-md border border-dashed border-auto-border-strong px-2 py-2 font-mono text-[10px] uppercase tracking-wide text-auto-text-secondary opacity-0 transition-opacity duration-200 hover:border-auto-accent hover:text-auto-accent group-hover/row:opacity-100 focus-visible:opacity-100 focus-visible:outline-none"
