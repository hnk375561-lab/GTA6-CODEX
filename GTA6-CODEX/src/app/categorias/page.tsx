      ...
      <div className="container-max relative">
        <Reveal className="mb-10">
          <nav className="mb-4 text-sm text-auto-text-secondary" aria-label="Breadcrumb">
            ...
            <span className="text-auto-text">Categorías</span>
          </nav>

          <h1 className="text-4xl font-bold text-auto-text">Categorías de vehículos</h1>
        ...
        {categories.length === 0 ? (
          ...
          <Reveal className="stagger grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {categories.map(({ group, count }) => (
              <Link key={group} href={`/categorias/${categoryToSlug(group)}`}>
                <Card hoverable className="h-full transition-colors duration-300 hover:border-auto-accent/60">
                  <CardBody className="flex h-full flex-col gap-2">
                    ...
                    <h2 className="text-xl font-bold text-auto-text">{group}</h2>
