 ...
 * Creates headlights and taillights moving along the road.
 */
...
export interface TrafficBuilderOptions {
  farGroup: THREE.Group
  quality: QualityProfile
}

export function buildTrafficStreaks(options: TrafficBuilderOptions): Updater {
  ...
  const updater: Updater = (_elapsed, delta, intro, _dayPhase, _humidity, _fog, entityPace, _entityUnrest, scrollVelocity, _pointerIntent, _entityPresence) => {
    streaks.forEach((s) => {
      s.mesh.position.z += s.dir * s.speed * (entityPace || 1) * delta * intro
      if (s.mesh.position.z > 30) s.mesh.position.z = -60
      if (s.mesh.position.z < -60) s.mesh.position.z = 30
      s.mesh.scale.y = 1 + Math.min((scrollVelocity || 0) * 40, 3)
    })
  }
